    ...
    return (
      <Card className="bg-card/70 border-accent shadow-xl text-center">
        ...
        <CardContent className="py-8">
          ...
          <p className="text-blue-200">Nenhum sorteio no histórico ainda.</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <Card className="bg-card/80 border-accent shadow-xl">
      <CardHeader className="flex flex-row justify-between items-center">
        <CardTitle className="text-3xl font-headline text-white flex items-center">
          <CalendarDays className="mr-3 h-8 w-8 text-primary" />
          Histórico de Sorteios
        </CardTitle>
            ...
            <AccordionItem value={entry.id} key={entry.id} className="bg-card/70 border-accent rounded-md shadow-md">
              <AccordionTrigger className="p-4 hover:no-underline">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center w-full text-left">
                  <div className="font-medium text-lg text-primary-foreground">
                    {format(new Date(entry.timestamp), "dd/MM/yyyy 'às' HH:mm", { locale: ptBR })}
                  </div>
                  <div className="text-sm text-blue-200 mt-1 md:mt-0">
              ...
              <AccordionContent className="p-4 pt-0">
                <div className="space-y-4">
                  <div>
                    <h4 className="text-lg font-semibold text-white mb-2 flex items-center"><Settings className="mr-2 h-5 w-5 text-primary"/>Configurações Usadas:</h4>
                    <div className="text-sm text-blue-100 space-y-1 ml-2">
                  ...
                  <div>
                    <h4 className="text-lg font-semibold text-white mb-2 flex items-center"><ListChecks className="mr-2 h-5 w-5 text-primary"/>Equipes:</h4>
                            ...
                            {team.length > 0 ? (
                              <ul className="space-y-1">
                                {team.map((player, playerIdx) => (
                                  <li key={playerIdx} className="flex items-center text-foreground text-sm p-1.5 rounded bg-white/5">
